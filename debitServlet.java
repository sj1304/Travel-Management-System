...
public class debitServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // Query to check if the card exists and fetch balance
            query = "SELECT balance FROM debit WHERE card_holder = ? AND card_no = ? AND expire_date = ? AND cvc = ?";
            ps = con.prepareStatement(query);
            ...
            if (rs.next()) {
                ...
                if (balance >= cost) {
                    ...
                    String updateBalanceQuery = "UPDATE debit SET balance = ? WHERE card_holder = ? AND card_no = ?";
                    ...
                    String updateBookingQuery = "UPDATE booking SET status = 'Paid' WHERE bid = ?";
                    ...
                    // Update package availability (reduce no_left)
